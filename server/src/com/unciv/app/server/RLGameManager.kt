...
object RLGameManager {
     ...
     *
     * **Safety**: after [removeGame] removes the game state, the old [gameId]
     * is a dead UUID that will never be reused (game IDs are generated by
     * [GameStarter] as random UUIDs).  Any request that calls [getMutex] for
     * the old ID after [cleanupMutex] will create a fresh [Mutex], acquire it
     * immediately, find `getGame == null`, and return 404 — so no correctness
     * issue arises even in this unlikely window.
     */
